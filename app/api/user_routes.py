...
@router.post(
    "/users",
    response_model=user_schema.UserCreated,
    status_code=201,
    responses=get_responses([201, 400, 401, 403, 409, 422, 426, 500]),
    tags=["Users"],
    description=f"Create an activated User + Password generated randomly. Permission={consts.Consts.PERMISSION_ADMIN}",
)
...
def create_user_as_admin(user: user_schema.UserCreate, request: Request, db: Session = Depends(get_db)):
    ...
    if db.query(user_model.User).filter_by(email=user.email).first():
        raise CustomException(
            ...
            info=f"User with same email {user.email} already exists"
        )

    # Check for existing phone
    if db.query(user_model.User).filter_by(phone=user.phone).first():
        raise CustomException(
            db=db,
            status_code=consts.Consts.ERROR_CODE_409,
            detail="Phone already registered",
            info=f"User with same phone {user.phone} already exists"
        )
...
@router.get(
    "/users/{user_id}",
    response_model=user_schema.UserPublicInfo,
    tags=["Users"],
    responses=get_responses([404, 426, 500]),
    description="Get a User. Permission=None"
)
@custom_declarators.version_check
...
@router.get(
    "/users",
    response_model=user_schema.UserPublicInfoList,
    tags=["Users"],
    responses=get_responses([426, 500]),
    description="List Users. Permission=None"
)
@custom_declarators.version_check
def list_users(
    request: Request,
    ...
    page: Optional[int] = 1,
    limit: Optional[int] = consts.Consts.MAX_RESULTS_PER_PAGE
):
...
@router.patch(
    ...
    response_model=user_schema.UserPublicInfo,
    tags=["Users"],
    responses=get_responses([400, 401, 403, 404, 409, 422, 426, 500]),
    description="Update a User. Permission=Admin or User Owner"
)
@custom_declarators.version_check
...
def update_user(user: user_schema.UserUpdate, user_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if user.username:
        db_user_with_username = user_crud.get_user_by_username(db, user.username)
        if db_user_with_username and db_user_with_username.id != user_id:
            raise CustomException(
                db=db,
                status_code=consts.Consts.ERROR_CODE_409,
                detail=consts.Consts.USER_ALREADY_EXIST,
    ...
    # Optionally, add similar checks for email and phone for update (not required unless you want to support email/phone updates)
...
@router.delete(
    ...
    tags=["Users"],
    responses=get_responses([401, 403, 404, 409, 422, 426, 500]),
    description="Disable a User. Permission=Admin or User Owner"
)
@custom_declarators.version_check
@custom_declarators.permission(permission_string=consts.Consts.PERMISSION_ADMIN_OR_USER_OWNER)
    ...
    responses={
            ...
            "content": {
                "application/json": {
                    "example": {
                        ...
                        "role_id": 2,
                        "activated": True,
                        "created_at": "2025-05-22T07:10:00",
                        "updated_at": None,
                        "full_name": "New User"
                    }
                }
        ...
        409: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": "User with same username newuser already exists"
                    }
                }
            }
        },
        ...
        500: {
            ...
            "content": {
                "application/json": {
                    "example": {
                        "detail": "Internal error: Failed to create user"
                    }
                }
            }
        }
...
def register_user(user: user_schema.UserRegister, db: Session = Depends(get_db)):
    # Check for existing username
    db_user = user_crud.get_user_by_username(db, username=user.username)
    if db_user:
        raise CustomException(
    ...
    db_role = role_crud.get_role_by_name(db, consts.Consts.ROLE_USER)
    if not db_role:
        raise CustomException(
            db=db,
            status_code=consts.Consts.ERROR_CODE_400,
            detail=consts.Consts.ROLE_NOT_FOUND,
            info=f"Role {consts.Consts.ROLE_USER} not found"
        )

    # Create User
    created_user = user_crud.register_user(db=db, user=user, default_role_id=db_role.id)
    return created_user